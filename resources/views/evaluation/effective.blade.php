<table class="table table-striped">
    ...
    <tbody class="effectiveness">
        ...
        @foreach($sectionII as $index => $two)
            <tr>
                <td width="60px" class="text-right">{{ $index + 1 }}</td>
                <td>{{ substr($two->name, stripos($two->name, ' ')) }}</td>
                <td class="text-center">{{ $two->score1 }}</td>
                <?php $totalII1+= $two->score1; ?>
                <td class="text-center">{{ $two->score2 }}</td>
                <?php $totalII2+= $two->score2; ?>
                <td class="text-center">{{ $two->score3 }}</td>
                <?php $totalII3+= $two->score3; ?>
                <td class="text-center">{{ $two->total }}</td>
                <?php $totalII4+= $two->total; ?>
            </tr>
        @endforeach
    ...
    <div id="lastTotal" data-url="{{ url('/') }}/evaluation/effective" style="display: none;">{{ $grandTotal4 }}</div>
</table>